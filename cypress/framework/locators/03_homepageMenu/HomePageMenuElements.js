class HomePageMenuElements {
    static cssBlock_CmsComponent = ".yCmsComponent ";
    static cssButton_CreateAccount = "ul.navigation-top__list.navigation-top__list--right > li:nth-child(2)";
    static cssButton_SignOut = "a[class='navigation-top__links-trigger']";
    static cssButton_Cart = ".yCmsContentSlot.miniCartSlot";
    static cssButton_MiniCart = " li.navigation-top__list-item.navigation-top__list-item--cart a";
    static cssLink_EpsonLogo = ".navigation__column.navigation__column--logo a";
}
export default HomePageMenuElements